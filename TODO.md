...
- [ ] The status of a camera can be no live signal yet. And when starting a project we are looking at the status of these cameras of course, And then it has it can have the status no live signal yet, while it did receive a live photo. So can we make the status active no live signal yet or inactive based on the images and daily reports instead of only daily reports. I'm not sure how it works currently, but it is worth an investigation to see how it works. 
