...
- [ ] Deleting a camera that has bulk upload jobs crashes with a raw 500. The cascade removes images, detections and classifications first, then dies on the bulk_upload_jobs foreign key, so the user sees "Internal Server Error" with no idea what went wrong or what was already deleted. Give it a clean 4xx that names the blocking jobs, and decide what deleting such a camera should do with them.
