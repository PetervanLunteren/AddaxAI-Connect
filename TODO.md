...
- [ ] improve the detections email. 

- [ ] Add delivery-worker liveness to the infrastructure health checks. The problem, discovered 7 Aug 2026 during the camera alerts bug hunt: the notifications-email container on the dev server had been down since 13 July without anyone noticing. Every notification email (reports, alerts, reminders) queued into Redis and sat there for weeks; nothing surfaced the outage because the queue accepts messages whether or not a consumer is alive, and the daily infra alert only checks disk usage and backup freshness. The same blind spot exists for the notifications-telegram worker, and the infra alert itself is sent by email, so a dead email worker also silences the very alert that should report it. The proposed fix has three parts. First, each delivery worker writes a heartbeat key to Redis (for example email_worker:last_seen) every consume-loop iteration, cheap and simple. Second, the hourly disk-usage check (or the daily infra alert) also verifies the heartbeats are fresh and the notification queues are not growing unbounded, and includes the result in the health endpoint so the server health page shows it. Third, because email cannot announce its own death, the alert for a dead email worker must go through a different channel, Telegram to the server admins, or at minimum a loud entry on the health page. Also check the prod servers' delivery workers by hand before this lands. Estimated one to two hours plus e2e.
