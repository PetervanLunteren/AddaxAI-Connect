...
2. Enable bucket **versioning** in the Wasabi console.
3. Create a **bucket-scoped IAM access key** with these permissions only: `s3:ListBucket`, `s3:GetObject`, `s3:GetObjectVersion`, `s3:PutObject`. No `DeleteObject`; Wasabi's lifecycle rule handles expiration, and the backup script never needs to delete.
4. Fill these ansible vars in `group_vars/dev.yml` (vault-encrypt the credentials):
   ```
   backup_enabled: true
   ...
   backup_secret_key: "..."
   ```
5. Run `ansible-playbook -i inventory.yml playbook.yml --tags sync-config`. This writes the new `BACKUP_*` lines to `.env` on the server and installs the cron.
6. Install the 90-day retention rule on the backup bucket. From inside the `minio` container:
   ```
   ...
   docker compose exec minio mc ilm rule add \
     ...
     backup-target/addaxai-connect-backups-prod
   ```
