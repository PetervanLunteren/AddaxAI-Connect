...
When the server can read the camera ID from a rejected file (missing or invalid GPS, missing date), the file also counts against that camera. The Cameras page shows a `Rejected files (7 days)` column and a needs attention chip for cameras that rejected a file in the last 7 days. The camera panel shows the 30-day total on its Overview and a `Rejected` tab with every file still kept and its reason. The column looks at the last 7 days on purpose: nearly every camera sends one photo before its first GPS fix, and that old setup shot should not make the camera look broken. Every project member sees this; the pictures are blurred when the project hides people or vehicles. Files without a readable camera ID (no metadata, unknown camera model) only show on `File management`.
