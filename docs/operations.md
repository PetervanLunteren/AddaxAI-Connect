...
When the server can read the camera ID from a rejected file (missing or invalid GPS, missing date), the file also counts against that camera. The Cameras page shows a `Rejected files` column and a needs attention chip for cameras with rejected files. Both only count the last 7 days, on purpose: nearly every camera sends one photo before its first GPS fix, and that old setup shot should not make the camera look broken. The camera panel shows the same number on its Overview, and a `Rejected` tab with those recent files open and the older ones (everything still kept) collapsed below them. Every project member sees this; the pictures are blurred when the project hides people or vehicles. Files without a readable camera ID (no metadata, unknown camera model) only show on `File management`.
...
3. Fill these ansible vars in `host_vars/<server>.yml`:
   ...
   backup_secret_key: "..."
   ```
4. Run `ansible-playbook -i inventory.yml playbook.yml --limit <server> --tags sync-config`, then `docker compose up -d --force-recreate` on the server. This writes the new `BACKUP_*` lines to `.env`, installs the daily cron, and restarts the containers with them.
