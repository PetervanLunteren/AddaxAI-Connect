...
export const DemographicChart: React.FC<DemographicChartProps> = ({
  ...
  const [field, setField] = useState<'sex' | 'life_stage'>('sex');
  ...
  const colorMap = field === 'sex' ? SEX_COLORS : LIFE_STAGE_COLORS;
      ...
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between flex-wrap gap-2">
          ...
          <div className="flex items-center gap-2">
            <Select
              value={field}
              onValueChange={(v) => setField(v as 'sex' | 'life_stage')}
        ...
        <p className="text-sm text-muted-foreground">
          {field === 'sex' ? 'Sex' : 'Life stage'} distribution from verified observations
