...
class TestSplitIntoSegments:
    ...
    def test_one_day_silence_keeps_segment(self):
        # Jan 1, then skip Jan 2, then Jan 3. Gap of 1 day. Stays merged.
        out = split_into_segments([date(2024, 1, 1), date(2024, 1, 3)])
        assert out == [(date(2024, 1, 1), date(2024, 1, 3))]

    def test_two_day_silence_keeps_segment(self):
        # Jan 1, skip Jan 2 and Jan 3, then Jan 4. Gap of 2 days. Stays merged.
        out = split_into_segments([date(2024, 1, 1), date(2024, 1, 4)])
        assert out == [(date(2024, 1, 1), date(2024, 1, 4))]

    def test_three_day_silence_splits(self):
        # Jan 1, then Jan 5. Three silent days (Jan 2, 3, 4). Splits.
    ...
    def test_four_day_silence_splits(self):
        out = split_into_segments([date(2024, 1, 1), date(2024, 1, 6)])
        assert out == [
            (date(2024, 1, 1), date(2024, 1, 1)),
    ...
    def test_multiple_segments(self):
        days = [
            date(2024, 1, 1),
            date(2024, 1, 3),    # 1-day gap, merge
            date(2024, 1, 4),
            date(2024, 1, 10),   # 5-day gap, split
            date(2024, 1, 11),
            date(2024, 1, 20),   # 8-day gap, split
        ]
    ...
    def test_max_gap_constant_matches_three_day_rule(self):
        # The constant is 2 because a 3-day silence corresponds to a gap
        # of `(d2 - d1).days - 1 == 3`, which must exceed the constant to
        # split. If this assertion fires, the docstring and copy will need
        # to be re-checked.
        assert MAX_INNER_BAR_GAP_DAYS == 2
