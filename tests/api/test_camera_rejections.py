"""Tests for the per-camera rejected file counts."""
...
from utils.camera_rejections import fetch_rejection_counts  # noqa: E402
...
async def test_counts_keyed_by_camera_and_missing_means_zero():
    ...
    counts = await fetch_rejection_counts(db, [1, 2, 3])
...
async def test_one_grouped_query_over_the_camera_ids():
    db = _FakeSession([])
    await fetch_rejection_counts(db, [7, 8])
...
async def test_no_cameras_means_no_query():
    db = _FakeSession([])
    assert await fetch_rejection_counts(db, []) == {}
