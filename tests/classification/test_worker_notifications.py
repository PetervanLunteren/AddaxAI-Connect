...
Migrated from services/classification/test_worker_notifications.py.
"""
import pytest
...
def filter_species_map(classifications, detection_confidence, det_threshold):
    """
    Reproduce the species_map filtering logic from worker.py.
...
def filter_annotation_pairs(classifications, detections, det_threshold):
    """
    Reproduce the annotation pair filtering logic from worker.py.
...
def build_notification_payload(species, classification, detection_confidence, camera_project_id=1):
    """
    Reproduce the notification event payload from worker.py.
...
class TestNotificationPayload:
    ...
    def test_payload_includes_detection_confidence(self):
        ...
        assert payload["detection_confidence"] == 0.92
